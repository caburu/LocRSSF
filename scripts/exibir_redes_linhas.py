# Exibe duas soluções do problema LocRSSF ligando os nós correspondentes
# por uma linha
#
...
def obter_nomes_arquivos(nro_arquivos = -1):
	...
	if len(sys.argv) > 1:
		...
		if nro_arquivos == -1:
			nro_arquivos = int(input("Informe a quantidade de arquivos: "))
		nomes_arqs = []
		for i in range(nro_arquivos):
...
NRO_ARQUIVOS = 2
nomes_arquivos = obter_nomes_arquivos(NRO_ARQUIVOS)
lado_area = obter_area()

# cores de plotagem (primeira letra é a cor, segunda indica que é um círculo)
estilo = "b,-"

coordenadas_x = [[]]*NRO_ARQUIVOS
coordenadas_y = [[]]*NRO_ARQUIVOS
for i in range(NRO_ARQUIVOS):
    l_x, l_y = le_arquivo_solucao(nomes_arquivos[i])
    coordenadas_x[i] = l_x
    coordenadas_y[i] = l_y

zip_coord_x = list(zip(coordenadas_x[0], coordenadas_x[1]))
zip_coord_y = list(zip(coordenadas_y[0], coordenadas_y[1]))

for i in range(len(zip_coord_x)):	
    plt.plot(zip_coord_x[i], zip_coord_y[i], estilo)
